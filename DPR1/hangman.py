...
def hangman(secret_word):
    ...
    * After each guess, you should display to the user the 
    ...
    while guesses_remaining > 0 and not is_word_guessed(secret_word, letters_guessed):
        ...
        is_invalid = not (guessed_letter.isalpha() and len(guessed_letter) == 1)
        is_already_guessed = guessed_letter in letters_guessed

        if is_invalid or is_already_guessed:
            if is_invalid:
...
def match_with_gaps(my_word, other_word):
    ...
    returns: boolean, True if all the actual letters of my_word match the
        ...
        False otherwise:
    '''
    my_word_list = tuple(my_word.replace(' ', ''))

    if len(my_word_list) != len(other_word):
        return False

    for i, letter in enumerate(other_word):
        if my_word_list[i] == letter:
            continue
        elif my_word_list[i] == '_' and letter not in my_word_list:
            continue
        return False
